...
class TestNotebookLMClientAuth:
    ...
    @pytest.mark.asyncio
    async def test_auto_retry_on_401(self, mock_client):
        ...
        with patch.object(mock_client, '_get_client', new_callable=AsyncMock) as mock_get_client, \
             patch.object(mock_client, '_refresh_auth_tokens', new_callable=AsyncMock) as mock_refresh:
            ...
            http_client.post = AsyncMock(side_effect=[
                httpx.HTTPStatusError("Unauth", request=req_401, response=resp_401),
                resp_200
            ])

            # Call RPC
            await mock_client._call_rpc("rLM1Ne", [])
    ...
    @pytest.mark.asyncio
    async def test_auto_retry_on_rpc_error_16(self, mock_client):
        ...
        with patch.object(mock_client, '_get_client', new_callable=AsyncMock) as mock_get_client, \
            ...
            # Execute
            result = await mock_client._call_rpc("rLM1Ne", [])
    ...
    async def test_get_notebook_sources_extracts_url(self, mock_client):
        ...
        with patch.object(mock_client, 'get_notebook', new_callable=AsyncMock, return_value=notebook_response):
            sources = await mock_client.get_notebook_sources_with_types("nb_uuid")
    ...
    async def test_add_drive_source_uses_extended_timeout(self, mock_client):
        ...
        with patch.object(mock_client, '_get_client', new_callable=AsyncMock) as mock_get_client, \
            ...
            req = httpx.Request("POST", "https://notebooklm.google.com/batchexecute")
            http_client.post = AsyncMock(return_value=httpx.Response(200, request=req, text="..."))

            mock_parse.return_value = []
            mock_extract.return_value = None

            await mock_client.add_drive_source("nb_id", "doc_id", "Title")
    ...
    async def test_add_drive_source_timeout_returns_status(self, mock_client):
        ...
        with patch.object(mock_client, '_get_client', new_callable=AsyncMock) as mock_get_client:
            ...
            result = await mock_client.add_drive_source("nb_id", "doc_id", "Title")
